...
export default function TabelaCursos() {
  ...
  function editar() {
    cursoApi
      .put(cursoAtt.cursoId, cursoAtt)
      .then((result) => abrirFecharModalEditar());
  }
  function deletar() {
    cursoApi
      .delete(cursoAtt.cursoId)
      .then((result) => abrirFecharModalDeletar());
  }

  const selecionar = (cursoAtt, opcao) => {
    setCursoAtt(cursoAtt);
    opcao == "Editar" ? abrirFecharModalEditar() : abrirFecharModalDeletar();
  };
          ...
          <tbody>
            {curso.map((curso) => (
              <tr key={curso.id}>
                ...
                <th>
                  <button
                    onClick={() => selecionar(curso, "Editar")}
                  ...
                  -
                  <button
                    onClick={() => selecionar(curso, "Deletar")}
      ...
      <Modal show={modalEditar}>
        ...
        <Modal.Body>
          <div className="form-group">
            ...
            <input
              readOnly
              value={cursoAtt && cursoAtt.cursoId}
              name="descricao"
              type="disable"
